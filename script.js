...
const ADV_OVERRIDE = /Approx\.?\s*quantity\s*in\s*meter\s*of\s*LHS\s*Cable\s*from\s*conveyor\s*to\s*deluge\s*valve\s*control\s*panel/i;
const RESULTS_OVERRIDE = /Design\s*Density\s*10\.?2\s*lpm\/?m2.*IS:?15325.*8\.7\.2/i;

// Split inputs into groups
const basicInputs = MODEL.inputs.filter(i => i.row > basicStart && i.row < advStart && !RESULTS_OVERRIDE.test(i.label||''));
const advInputs = MODEL.inputs.filter(i => (i.row > advStart || ADV_OVERRIDE.test(i.label||'')) && !RESULTS_OVERRIDE.test(i.label||''));
...
function renderResults(st) {
  ...
  // Show formula rows except BOM rows
  ...
  // Include "Design Density..." (read-only) even if it is an input
  const dd = MODEL.inputs.find(i => RESULTS_OVERRIDE.test(i.label||''));
  if (dd) {
    const row = document.createElement('div'); row.className = 'kv row';
