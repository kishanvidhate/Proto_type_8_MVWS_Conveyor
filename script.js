...
// Build default state from inputs
const defaultState = {};
...
function renderInputs() {
  ...
  MODEL.inputs.forEach(inp => {
    ...
    input.oninput = (e)=>{ const v = parseFloat(e.target.value); state[inp.address] = Number.isFinite(v) ? v : 0; };
    wrap.appendChild(label); wrap.appendChild(input); inputsDiv.appendChild(wrap);
  });
}
...
function computeAll() {
  ...
  // Evaluate formulas in sheet order
...
function renderResults(st) {
  ...
  const bomStart = 29, bomEnd = 47;
  const labels = Object.fromEntries(MODEL.formulas.map(f => [f.address, f.label]));
  MODEL.formulas.forEach(f => {
    const rowNum = parseInt(f.address.slice(1), 10);
    if (rowNum >= bomStart && rowNum <= bomEnd) return; // skip BOM rows from results
    const row = document.createElement('div'); row.className = 'row';
    const l = document.createElement('div'); l.className = 'label'; l.textContent = labels[f.address] || f.address;
    const v = document.createElement('div'); v.textContent = (st[f.address] !== undefined && st[f.address] !== null) ? String(st[f.address]) : '';
...
function renderBOM(st) {
  ...
  for (let r = 29; r <= 47; r++) {
    const addr = 'C' + r;
    const desc = labels[addr];
    if (!desc) continue;
    const qty = st[addr];
    if (qty === '' || qty === null || qty === undefined) continue;
    // show numeric or string as-is; no formatting
    if (!(typeof qty === 'number' && (Number.isNaN(qty) || qty === 0))) {
      const tr = document.createElement('tr');
      const tdq = document.createElement('td'); tdq.textContent = String(qty);
